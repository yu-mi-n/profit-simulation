...
ad_cost = int(st.sidebar.slider('広告費（万円）', 1000, 9000)*1.0E+04)
...
with cost_col1:
  st.write(f'固定費：{fix_cost}')
with cost_col2:
  st.write(f'広告宣伝費: {ad_cost}')

col1, col2, col3 = st.columns(3)
col1.metric('費用', f'{int(cost/1.0E+4)}万円', height='stretch')
col1.metric('最適な広告投下費用', f'{int(best_ad_cost)} 万円')
col2.metric('予想売上', f'{int(earning/1.0E+04)} 万円')
col3.metric('予想利益', f'{int(profit/1.0E+04)} 万円', f'{profit_ratio} %', height='stretch')
